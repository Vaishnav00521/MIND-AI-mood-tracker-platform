...
export const Survey = () => {
  ...
  const handleSubmit = () => {
    ...
    // Build a short journal text from the tail answers
    const qKeys = Array.from({ length: 20 }, (_, i) => t(`survey.q${i + 1}`));
    const journal = qKeys[19]; // Use the last question's context as summary
    submitSurvey(answers, journal);
    setTimeout(() => {
                  ...
                  {getOptions().map((opt) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex-1 py-4 px-2 rounded-2xl text-sm border transition-all duration-300 ${isSelected
                            ? opt.selected
